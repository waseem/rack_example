class BrowserCheckApp
  ...
  def ie_found_in?(env)
    if env['HTTP_USER_AGENT']
      is_ie?(env['HTTP_USER_AGENT']) and
      ie_version(env['HTTP_USER_AGENT']) < @options[:minimum] and
      @options[:redirect] != env['PATH_INFO'] # We do not want to redirect to requesting URI. It will make an infinite redirect cycle.
    end
  end
